...
class CycleGANclModel(BaseModel):
    ...
    def backward_G(self):
        ...
        # GAN loss D_B(G_B(B))
        self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_A_B), True)# + self.criterionGAN(self.netD_A(self.fake_A_C), True)
        # GAN loss D_C(G_C_A/B(C_A/B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_B_A), True)# + self.criterionGAN(self.netD_B(self.fake_B_C), True)
        # Forward cycle loss
        self.loss_cycle_A = self.criterionCycle(self.rec_A_B, self.real_A) * lambda_A# + self.criterionCycle(self.rec_A_C, self.real_A) * lambda_A
        # Backward cycle loss
