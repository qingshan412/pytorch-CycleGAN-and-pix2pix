...
class CycleGANcModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser.add_argument('--lambda_A', type=float, default=10.0, help='weight for cycle loss (A -> C -> A)')
            parser.add_argument('--lambda_B', type=float, default=10.0, help='weight for cycle loss (B -> C -> B)')
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'D_C_A', 'D_C_B', 'G_C_A', 'G_C_B', 'cycle_C_A', 'cycle_C_B', 'idt_C_A', 'idt_C_B']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'G_C_A', 'G_C_B', 'D_C']
        ...
        self.netG_C_A = networks.define_G(opt.input_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netG_C_B = networks.define_G(opt.output_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def backward_G(self):
        ...
        self.loss_G_C = (self.criterionGAN(self.netD_C(self.fake_C_A), True) + self.criterionGAN(self.netD_C(self.fake_C_B), True)) / 2.
        # GAN loss D_B(G_B(B))
        ...
        # Backward cycle loss
        self.loss_cycle_C = (self.criterionCycle(self.rec_C_A, self.real_C) + self.criterionCycle(self.rec_C_B, self.real_C)) * lambda_C / 2.
        # combined loss
