...
class CycleGANccModel(BaseModel):
    ...
    def forward(self):
        ...
        self.rec_B_A = self.netG_B_C(self.rec_C_fake_C_B)
    ...
    def backward_G(self):
        ...
        # GAN loss D_B(G_B(B))
        self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_A_C), True) + self.criterionGAN(self.netD_A(self.fake_A_B), True)
        # GAN loss D_C(G_C_A/B(C_A/B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_B_C), True) + self.criterionGAN(self.netD_B(self.fake_B_A), True)
        # Forward cycle loss
        self.loss_cycle_A = self.criterionCycle(self.rec_A_C, self.real_A) * lambda_A + self.criterionCycle(self.rec_A_B, self.real_A) * lambda_A
        # Backward cycle loss
        self.loss_cycle_B = self.criterionCycle(self.rec_B_C, self.real_B) * lambda_B + self.criterionCycle(self.rec_B_A, self.real_B) * lambda_B
        # Backward cycle loss
        self.loss_cycle_C = (self.criterionCycle(self.rec_C_A, self.real_C) * lambda_C + self.criterionCycle(self.rec_C_B, self.real_C) * lambda_C 
            + self.criterionCycle(self.rec_C_fake_C_A, self.fake_C_A) * lambda_C + self.criterionCycle(self.rec_C_fake_C_B, self.fake_C_B) * lambda_C)/2.
        # combined loss
